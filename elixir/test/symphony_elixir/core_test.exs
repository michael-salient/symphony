defmodule SymphonyElixir.CoreTest do
  ...
  test "current WORKFLOW.md file is valid and complete" do
    ...
    assert Map.get(workspace, "repository_url") == "https://github.com/openai/symphony"

    hooks = Map.get(config, "hooks", %{})
    assert is_map(hooks)
    assert Map.get(hooks, "after_create") =~ "SYMPHONY_TARGET_REPOSITORY_URL"
    assert Map.get(hooks, "after_create") =~ "git clone --depth 1 \"$SYMPHONY_TARGET_REPOSITORY_URL\" ."
